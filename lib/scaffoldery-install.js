...
request.get(zipUrl).pipe(fs.createWriteStream(zipFile)).on('close', function () {
  ...
  if (!scaffolderyJson.package) {
    scaffolderyJson.package = package;
  }
...
var undefined = path.join(scaffolderyPath, 'undefined-master.zip');
fs.exists(undefined, function (exists) {
  if (exists) fs.unlinkSync(undefined);
});
